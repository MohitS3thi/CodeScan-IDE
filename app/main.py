...
async def upload_image(file: UploadFile = File(...)) -> dict:
    ...
    if not is_supported_upload(filename):
        raise HTTPException(status_code=400, detail="Unsupported file type. Use PNG, JPG, JPEG, WEBP, HEIC, or PDF.")
...
async def recognize_handwritten_text(
    ...
    Args:
        ...
        context: Optional context (e.g., "Python code from whiteboard").
    ...
    if pipeline is None:
        raise HTTPException(
            status_code=503,
            detail="OCR pipeline not available. Please configure API key.",
        )
    ...
    if not is_supported_upload(filename):
        raise HTTPException(
            status_code=400,
            detail="Unsupported file type. Use PNG, JPG, JPEG, WEBP, HEIC, or PDF.",
        )
    ...
    return {
        ...
        "capabilities": {
            "phase1": {
                ...
                "features": [
                    "Image upload (PNG, JPEG, WEBP, HEIC, PDF)",
                    "Grayscale conversion",
                    "EXIF orientation correction",
                    "Image resizing with bounds",
